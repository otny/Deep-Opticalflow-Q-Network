...
from src.dqn_opticalflow_agent import DQNOPTICALFLOWAgent
...
class Main():
    ...
    def __init__(self, net_type, conf):
        ...
        elif net_type == "dqn_opticalflow":
            self.agent = DQNOPTICALFLOWAgent(conf)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--flickering", type=bool, default=True, help="flickering play")
    parser.add_argument("--flickering_p", type=float, default=0.5, help="flickering rate")
    parser.add_argument("--epsilon_wariai", type=float, default=0.7, help="last epsilon step rate of all steps")
    ...
    conf.epsilon_decay_episodes = args.steps*conf.epsilon_end_wariai
    conf.epsilon_decay = float((conf.epsilon_start - conf.epsilon_end))/float(conf.epsilon_decay_episodes)
    conf.flickering = args.flickering
    conf.flickering_p = args.flickering_p
